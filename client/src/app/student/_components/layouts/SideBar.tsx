...
import { STUDENT_SIDEBAR_LINKS } from '@/constants/sidebar'
...
const SideBar = () => {
  ...
  <div className="p-2 w-full flex gap-1 flex-col">
    {STUDENT_SIDEBAR_LINKS.map((s) => (
      <TooltipProvider key={s.label}>
        <Tooltip>
          <TooltipTrigger asChild>
            <Link
              href={s.href}
              className={`p-3 w-full flex items-center gap-3 rounded-lg transition duration-200 ease-in-out cursor-pointer ${
                isExpanse ? 'justify-start' : 'justify-center'
              } ${pathName === s.href ? 'bg-blue-500 text-white' : 'bg-background hover:bg-neutral-300'}`}
            >
              {/* Đảm bảo icon có cùng kích thước */}
              <div className="w-6 h-6 flex items-center justify-center">{s.icon}</div>
              {/* Chỉ hiển thị text khi sidebar mở rộng */}
              {isExpanse && <span className="text-[15px] font-medium">{s.label}</span>}
            </Link>
          </TooltipTrigger>
          {/* Chỉ hiển thị tooltip khi sidebar thu gọn */}
          {!isExpanse && (
