    ...
    <div className="flex flex-col gap-2 p-4 border rounded-lg shadow-md w-full h-full">
      ...
      {topics.map((topic) => (
        <div key={topic.id} className="p-3 border rounded-lg bg-gray-100 w-full relative">
          ...
          <p className="text-sm"><strong>Requirements:</strong> {topic.requirements}</p>
          <p className="text-sm"><strong>Prerequisites:</strong> {topic.prerequisites}</p>
          <p className="text-sm"><strong>Guidelines:</strong> {topic.guidelines}</p>
